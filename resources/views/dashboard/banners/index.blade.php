...
@section('content')
 ...
 <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        ...
        <a href="{{ route('dashboard.banners.trashed') }}" class="d-sm-inline-block btn btn-sm btn-danger shadow-sm">
            <i class="fas fa-trash fa-sm text-white-50"></i>
            @lang('main.trashed') @lang('main.banners')
        </a>
    </div>
</div>
        ...
        <div class="col-xl-12 col-md-12 col-sm-12">
                <div class="card">
                    ...
                    <div class="card-body table-responsive">
                        <form method="post" action="{{ route('dashboard.banners.destroyGroup') }}">
                            @csrf
                            @method('DELETE')
                            
                            {!! $dataTable->table(['class'=> 'table table-sbannered table-bordered table-hover'], true) !!}
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.createBtn', function() {
            window.location = "{{ route('dashboard.banners.create') }}";
        });
