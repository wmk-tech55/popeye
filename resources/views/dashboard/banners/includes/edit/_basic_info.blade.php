...
<div class="form-group my-4 row">
    ...
    <div class="col-sm-10">
        <input type="text" name="en_name" value="{{ $banner->en_name }}" id="en_name" class="form-control @error('en_name') is-invalid @enderror" placeholder="@lang('main.en_title')" required>
        @error('en_name')
...
<div class="form-group mb-4 row">
    ...
    <div class="col-sm-10">
        <input type="text" name="ar_name" value="{{ $banner->ar_name }}" id="ar_name" class="form-control @error('ar_name') is-invalid @enderror" placeholder="@lang('main.ar_title')" required>
        @error('ar_name')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
    ...
    <div class="col-sm-10">
        <select name="store_id" id="store_id" class="form-control select2 @error('store_id') is-invalid @enderror" data-placeholder="@lang('main.company')" >
            ...
            @foreach ($stores as $store)
                <option value="{{ $store->id }}" {{ old('store_id', $banner->store_id) == $store->id ? 'selected' : '' }}> {{ $store->full_name }} </option>
            @endforeach
        </select>
