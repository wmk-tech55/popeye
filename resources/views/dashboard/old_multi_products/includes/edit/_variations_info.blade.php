{{--  @dd(old('type', $product->type) , $errors->all()) --}}
<div class="form-group mb-4 row">
    <label class="col-sm-2 col-form-label" for="type">@lang('main.product_type')<span class="required"></span> </label>
    <div class="col-sm-10">

        <div class="custom-control custom-radio custom-control-inline">
            <input type="radio" checked id="customRadio1" name="type" value="one_option"
                {{ old('type', $product->type) == 'one_option' ? 'checked' : '' }} class="custom-control-input">
        ...
        <div class="custom-control custom-radio custom-control-inline">
            <input type="radio" id="customRadio2" name="type" value="multiple_options"
                {{ old('type', $product->type) == 'multiple_options' ? 'checked' : '' }} class="custom-control-input">
    ...
    <div class="col-sm-10">
        ...
        <div class="special-price  " id="special-price">
            ...
            <div id="addVariationRowData">
                ...
                @if ($product->multipleOptionVariations->isNotEmpty())
                    @foreach ($product->multipleOptionVariations as $variation)
                      
                        <div class="special-price-container">
                            <div class="col-sm-3 d-inline-block">
                                <input type="text" name="variations_ar_names[]" value="{{ $variation->ar_name }}"
                                    class="form-control mb-1 special-price-inputs @error("variations_ar_names.{$loop->index}") is-invalid @enderror"
                                    placeholder="@lang('main.ar_name') *">
                            </div>
                            <div class="col-sm-3 d-inline-block">
                                <input type="text" name="variations_en_names[]" value="{{ $variation->en_name }}"
                                    class="form-control mb-1 special-price-inputs @error("variations_en_names.{$loop->index}") is-invalid @enderror"
                                    placeholder="@lang('main.en_name') *">
                            </div>
                            <div class="col-sm-3 d-inline-block">
                                <input type="number" name="variations_prices[]" value="{{ $variation->price }}"
                                    class="form-control mb-1 special-price-inputs @error("variations_prices.{$loop->index}") is-invalid @enderror"
