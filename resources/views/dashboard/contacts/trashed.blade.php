...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            <a href="{{ route('dashboard.admins.create') }}" class="d-sm-inline-block btn btn-sm btn-success shadow-sm">
                <i class="fas fa-plus fa-sm text-white-50"></i>
                @lang('main.add') @lang('main.admins')
            </a>
            <a href="{{ route('dashboard.admins.index') }}" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                <i class="fas fa-plane fa-sm text-white-50"></i>
                @lang('main.show_all') @lang('main.admins')
            </a>
        </div>
    </div>
        ...
        <div class="col-xl-12 col-md-12 col-sm-12">
            <div class="card">
                ...
                <div class="card-body table-responsive">
                    <form method="post" action="{{ route('dashboard.admins.forceDestroyGroup') }}">
                        @csrf
                        @method('DELETE')
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.createBtn', function() {
            window.location = "{{ route('dashboard.admins.create') }}";
        });
